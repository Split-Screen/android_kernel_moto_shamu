 ...
 *	      (C)  2013-14 flar2 <asegaert@gmail.com>
 *
...
static inline void switch_input_mode(unsigned timeout)
{
	if (timeout > 0)
		mod_timer(&freq_mode_timer, jiffies + msecs_to_jiffies(timeout));
	tbl_select[0] = 2;
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (g_count > 30) {
		...
		if (max_load_freq > dbs_tuners_ins.up_threshold * policy->cur) {
			...
			if (dbs_tuners_ins.two_phase_freq != 0 && phase == 0) {
				...
				if (policy->cur < policy->max && g_count > 80)
	...
	if (dbs_tuners_ins.gboost) {
		...
		if (g_count < 100 && graphics_boost < 4) {
			++g_count;
		} else if (g_count > 1) {
			--g_count;
			--g_count;
		...
		unsigned int code, int value)
{
	...
	if (type == EV_ABS && code == ABS_MT_TRACKING_ID) {
		if (value != -1) {		
			...
			switch_input_mode(DBS_SWITCH_MODE_TIMEOUT);
		}
	}
}
